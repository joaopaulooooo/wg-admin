...
if [[ -d "$CERT_DIR" && -r "$CERT_DIR/fullchain.pem" && -r "$CERT_DIR/privkey.pem" ]]; then
  ...
  # certbot standalone (HTTP-01) precisa da porta 80/tcp. Abrimos antes do
  # certbot e mantemos aberta — o `certbot.timer` precisa dela de novo a cada
  # renovação (a cada ~60 dias). ufw allow / firewall-cmd --permanent são
  # idempotentes e persistem entre reboots.
  if command -v ufw >/dev/null && ufw status 2>/dev/null | grep -q "Status: active"; then
    if ! ufw status 2>/dev/null | grep -qE "(^|[^0-9])80/tcp"; then
      info "ufw ativo — a abrir 80/tcp (Let's Encrypt e renovações)"
      ufw allow 80/tcp || true
    fi
  fi
  if command -v firewall-cmd >/dev/null && firewall-cmd --state 2>/dev/null | grep -q "running"; then
    if ! firewall-cmd --list-ports 2>/dev/null | tr ' ' '\n' | grep -qx "80/tcp"; then
      info "firewalld ativo — a abrir 80/tcp (Let's Encrypt e renovações)"
